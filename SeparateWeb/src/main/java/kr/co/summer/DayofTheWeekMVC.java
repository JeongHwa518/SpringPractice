...
public class DayofTheWeekMVC {
	...
	public ModelAndView kosta(int year, int month, int day) {
		...
		mv.setViewName("dayofweek");	 // dayofweek 은 .jsp를 말함
	...
	private boolean isValid(int year, int month, int day) {
		if(year == -1 || month == -1 || day == -1)		// 셋 중 하나라도 유효하지 않을 경우
			return false;
	...
	private char getDayOfWeek(int year, int month, int day) {
		...
		cal.set(year, month-1, day);   //month는 0부터 시작하므로 -1 (날짜 설정)
		
		int dayofWeek = cal.get(Calendar.DAY_OF_WEEK);	 // DAY_OF_WEEK 요일을 반환 (sunday = 1,...)
